...
public class GoogleStorageClient implements DestinationStorage {
  ...
  public void createObject(String path, String mimeType, InputStream in, boolean dryRun) {
    ...
    logger.atInfo().log(
        "Storing %s content to location %s.",
        Preconditions.checkNotNull(mimeType, "Mime type can't be null."), path);
    if (dryRun) {
      logger.atInfo().log(
          "Skip actual copying of %s file to %s since it's dry run.", mimeType, path);
      return;
    }
    storage.create(
  ...
  public Set<String> listObjects(String path) {
    return StreamSupport.stream(
            storage.list(bucketName, BlobListOption.prefix(path)).iterateAll().spliterator(), false)
        .map(b -> new File(b.getName()).getName())
